...
PROD_DB_NAME = 'm_urlqueue_dev'
PROD_META_DB = 'm_MMCTRP'
DB_PASSWORD = 'root'

def ensure_content_table(curs, source):
    mmt_table = '%s' %(source)
    SHOW_QUERY = 'SHOW TABLES LIKE "%s";' %(source)
    curs.execute(SHOW_QUERY)
    try:curs.execute(CTCONTENT_TABLE_CREATE_QUERY.replace('#CONTENT-TABLE#', source))
    except: print "Table already exists"

def ensure_booking_table(curs, source):
    gbt_table = '%s' %(source)
    SHOW_QUERY = 'SHOW TABLES LIKE "%s";' %(source)
    curs.execute(SHOW_QUERY)
    try:curs.execute(BOOKING_TABLE_CREATE_QUERY.replace('#BOOKING-TABLE#', source))
    except: print "Table already exists"
...
def terminal_staticrequests(cursor, source, crawl_type, content_type, limit):
    crawl_table_name = "%s_crawl" % (source)
    if limit:
        cursor.execute(CRAWL_TABLE_SELECT_QUERY_LIMIT_STATIC%(crawl_table_name, crawl_type, content_type, limit))
    else:
        cursor.execute(CRAWL_TABLE_SELECT_QUERY_STATIC%(crawl_table_name, crawl_type, content_type))
...
def terminal_tripadvisor_requests(cursor, source, crawl_type, content_type, dx_val, limit):
    ...
    if limit:
        if dx_val:
            ...
            cursor.execute(TA_TABLE_SELECT_AQUERY_LIMIT_WDX%(crawl_table_name, crawl_type, content_type, limit))
    else:
        if dx_val:
            cursor.execute(TA_TABLE_SELECT_QUERY%(crawl_table_name, crawl_type, content_type, dx_val))
        else:
            cursor.execute(TA_TABLE_SELECT_QUERY_WDX%(crawl_table_name, crawl_type, content_type))
    ...
    if not rows:
        if limit:
                if dx_val:
                    ...
                    cursor.execute(TA_TABLE_SELECT_QUERY_LIMIT_WDX%(crawl_table_name, crawl_type, content_type, limit))
                rows = cursor.fetchall()
    sks_lst = []
    if rows:
        for row in rows:
...
def terminal_trivago_requests(cursor, source, crawl_type, content_type, limit):
    crawl_table_name = "%s_crawl" % (source)
    if limit:
        cursor.execute(TG_TABLE_SELECT_QUERY_LIMIT%(crawl_table_name, crawl_type, content_type, limit))
    else:
        cursor.execute(TG_TABLE_SELECT_QUERY%(crawl_table_name, crawl_type, content_type))
    rows = cursor.fetchall()
    sks_lst = []
    if rows:
        for row in rows:
...
def ensure_ta_table(curs, source):
    gbt_table = '%s' %(source)
    SHOW_QUERY = 'SHOW TABLES LIKE "%s";' %(source)
    curs.execute(SHOW_QUERY)
    try:curs.execute(TRIPM_TABLE_CREATE_QUERY.replace('#TA-TABLE#', source))
    except: print "Table already exists"
    try:curs.execute(TRIPMIN_TABLE_CREATE_QUERY.replace('#TA-TABLE#', source))
    except: print "Table already exists"
    if 'APC' not in source :
	    try:curs.execute(TRIPMCITY_TABLE_CREATE_QUERY.replace('#TA-TABLE#', source))
	    except: print "Table already exists"



def create_mmt_table_cusor():
    conns = MySQLdb.connect(host ='localhost', user ='root', db = PROD_META_DB, charset='utf8', use_unicode=True, passwd = DB_PASSWORD)
    curs = conns.cursor()
    return curs
...
def terminal_contentclear_requests(cursor, source, crawl_type, content_type, limit):
    crawl_table_name = "%s_crawl" % (source)
    if limit:
        cursor.execute(CONTENT_CLEAR_TABLE_SELECT_QUERY_LIMIT%(crawl_table_name, crawl_type, content_type, limit))
    else:
        cursor.execute(CONTENT_CLEAR_TABLE_SELECT_QUERY%(crawl_table_name, crawl_type, content_type))
    rows = cursor.fetchall()
    sks_lst = []
    if rows:
        for row in rows:
...
def ensure_crawlta_table(cur, source):
	crawl_table = '%s_crawl'%(source)
	SHOW_QUERY = 'SHOW TABLES LIKE "%s_%%";' %(source)
	cur.execute(SHOW_QUERY)
	try:cur.execute(TA_CRAWL_TABLE_CREATE_QUERY.replace('#CRAWL-TABLE#', crawl_table))
	except Exception, e: print str(e)

def drop_crawlta_table(cur,source):
	dropcrawl_table ='%s_crawl'%(source)
	DROP_QUERY ='TRUNCATE TABLE %s;' %(dropcrawl_table)
	try:cur.execute(DROP_QUERY)
	except Exception, e: print str(e)

def drop_ta_table(cur, source):
	...
	DROP_QUERY2 = 'TRUNCATE TABLE %s.%sInventory' % (db_name, source)
	try:
	    cur.execute(DROP_QUERY2)
	except Exception, e:
	    print str(e)

def insert_crawlta_tables_data(cur, source, val):
	CRAWL_TABLE_QUERY='INSERT INTO %s_crawl'%source +'(sk, url, crawl_type, start_date,'
