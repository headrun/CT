...
class TripAdvisor(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        if rows:
            for row_inner in rows:
                ...
                counter_number = 0
                while index_count:
                    trip_advisor_url = "https://www.tripadvisor.in/MiniMetaCRAjax?detail=%s&staydates=%s_%s&rooms=%s&adults=%s&child_rm_ages=&area=QC_Meta_Mini&returnAllOffers=true&imp=true&metaReferer=Hotel_Review&baseLocId=%s&metaRequestTiming=%s&finalRequest=true&disableLazyLoading=true" % (
    ...
    def parse_vendor(self, response):
        ...
        if 'makemytrip.com' in redirect_url:
            ...
            transfer_meta.update({"cheapest_room_name":"NA", "cheapest_ota_price":"NA","status":status, "is_going_to_ct":"yes", "vendor_key":"cleartrip", "cheaperst_url":response.url})
    ...
    def parse_vendordetails(self, response):
        ...
        if vendor_key == "goibibo":
            room_name_inf = re.findall('type_name\":\"(.*?)\"', response.body)
            if room_name_inf:
                room_name = room_name_inf[0]
        ...
        if vendor_key == "booking":
            ...
            room_fin  = sel.xpath('//span[contains(@class, "roomtype-icon-")]//text()').extract()
            if room_fin:
        ...
        if transfer_meta.get('ct_url', ''):
            ...
            if transfer_meta.get('is_going_to_ct', ''):
                transfer_meta.update({'cleartrip_room_name':room_name, "cleartrip_inner_message":status_inner, "vendor_key":vendor_key, "ct_ta_url":response.url})
	...
	cheapest_to_returninner = ''
        if ('goibibo' or 'booking' or 'makemytrip') not in cheapest_to_return[-1].lower():
		cheapest_to_returninner = [(int(val[0]), val[1]) for val in update_cheapest_three if val[0] != '0' and (val[1].lower() == 'makemytrip' or val[1].lower() == "makemytripintl") and val[0] != 0]
		if not cheapest_to_returninner:
			cheapest_to_returninner = [(int(val[0]), val[1]) for val in update_cheapest_three if val[0] != '0' and (val[1].lower() == 'goibibotrip' or val[1].lower() == "goibibointl") and val[0] != 0]
		if not cheapest_to_returninner:
			cheapest_to_returninner = [(int(val[0]), val[1]) for val in update_cheapest_three if val[0] != '0' and (val[1].lower() == 'bookingtrip' or val[1].lower() == "bookingintl") and val[0] != 0]
		if not cheapest_to_returninner:
    ...
    def yield_vendor(self, transfer_meta):
        ...
        ct_pric_her, cheapest_ota_data, cheapest_pric_her, cheapest_ota_name = ['']*4
	...
	if transfer_meta.get('cheapest_proper_ota', ''):
        ...
        to_yield.update({"sk": transfer_meta.get('sk', ''), "city_name":transfer_meta.get('city_name', ), "property_name": transfer_meta.get('propert_name', ''), "TA_hotel_id":transfer_meta.get('TA_hotel_id', ''), "CT_hotel_id":transfer_meta.get('ctid', ''), "checkin":transfer_meta.get('checkin', ''), "DX":transfer_meta.get('dx', ''), "Pax":transfer_meta.get('pax', ''), "ct_price":ct_pric_her, "cheapest_ota_price":cheapest_pric_her, "cheapest_ota_name":normalize_clean(cheapest_ota_name), "ct_room_name":normalize_clean(transfer_meta.get('cleartrip_room_name', '')), "cheapest_ota_room_name":normalize_clean(transfer_meta.get('cheapest_room_name', '')), "status":normalize_clean(status),"which_case":vendor_prices,  "Ranking_ClearTrip":transfer_meta.get('Ranking_ClearTrip', ''), "Time":transfer_meta.get('average_3_cheapest_ots' , ''), "reference_url":transfer_meta.get('cheaperst_url', ''), "main_url":transfer_meta.get('which_case', ''), "from_ta_url":vendor_names})
        return to_yield
            ...
            if (check_loading and '\PP:' in tracking_check) or (check_loading):
                ...
                if final_dict:
                    ...
                    ct_pric = [chct for chct in filter_0and_ if chct[1] ==
                               'ClearTrip' and chct[0] != '0' and chct[0] != '-']
                    if not ct_pric:
                        ct_pric = [chct for chct in filter_0and_ if chct[1] == 'ClearTripIntl' and chct[0] != '0' and chct[0] != '-']
                    if ct_pric:
                        ct_data_re = (ct_pric[0][-1], ct_pric[0][1])
                    else:
                        ct_data_re = ''
                    cleartrip_ranking_inventory = final_dict.get('ClearTrip', '')
                    ...
                    meta_dict = {"ct_data":ct_data_re, "sk":response.meta.get('sk', ''), "city_name":response.meta.get('city_name', ''), "propert_name":response.meta.get('hotel_name', ''), "TA_hotel_id":response.meta.get('hotel_id', ''), "ctid":response.meta.get('ctid', ''), "checkin":response.meta.get('dxs'), "dx":response.meta.get('DX_num'), "pax":response.meta.get('PAX_val'),  "Ranking_ClearTrip":cleartrip_ranking_inventory, "time_in":response.meta.get('full_time', '')}
			...
			uctt = sum([asa[0] for asa in update_cheapest_three])/3
                        ...
                        if dif_lo:
                            if dif_pr > float(dif_lo):
                                ...
                                cheapest_to_return, cheapest_proper_ota = self.cheapest_re(update_cheapest_three, cheapest_to_return)
                                ...
                                if final_dict_navigations[cheapest_to_return[-1]] and final_dict_navigations[cheapest_to_return_ct[-1]]:
                                    cturl_checkin_ = datetime.datetime.strftime((datetime.datetime.strptime(meta_dict.get('checkin', ''), '%Y-%m-%d')), '%d/%m/%Y')
                                    cturl_checkout = datetime.datetime.strftime((datetime.datetime.strptime(response.meta.get('loss', ''), '%Y_%m_%d')), '%d/%m/%Y')
                                    ct_url_ = "https://www.cleartrip.com/hotels/service/rate-calendar?chk_in=%s&chk_out=%s&num_rooms=1&adults1=2&children1=0&shwb=true&adults=2&childs=0&ct_hotelid=%s&pahCCRequired=true&bestprice=true" % (cturl_checkin_, cturl_checkout, ctid)
                                    meta_dict.update({"ct_url":ct_url_, "status":"success", "which_case":"case_one", "cheapest_ota_data":cheapest_to_return, "cheapest_proper_ota":cheapest_proper_ota, "average_3_cheapest_ots":uctt})
                                    ...
                                    meta_dict.update({"status":statuss_, "which_case":"case_one"})
                        ...
                        update_cheapest_three = sorted(
                            to_find_cheapest_three, key=itemgetter(0))[0:3]
			cheapest_to_return, cheapest_proper_ota = self.cheapest_re(update_cheapest_three, cheapest_to_return_one)
                        if final_dict_navigations[cheapest_to_return[-1]]:
                            meta_dict.update({"ct_url":'', "status":"success", "which_case":"case_two", "cheapest_ota_data":cheapest_to_return, "cheapest_proper_ota":cheapest_proper_ota})
                            yield Request(final_dict_navigations[cheapest_to_return[-1]], callback=self.parse_vendor, dont_filter=True, meta = meta_dict)
                        else:
                            statuss_ = 'Vendor url is not preset as static keys are changed, failed'
                            meta_dict.update({"status":statuss_, "which_case":"case_two"})
                            vendor_iyield = self.yield_vendor(meta_dict)
                            if vendor_iyield:
                                yield vendor_iyield
                    else:
                        statuss_ = 'we dont have atleast 3 or 4 cheapest OTA prices, failed'
                        meta_dict.update({"status":statuss_, "which_case":"case_one_two"})
                        vendor_iyield = self.yield_vendor(meta_dict)
                        if vendor_iyield:
                            yield vendor_iyield
