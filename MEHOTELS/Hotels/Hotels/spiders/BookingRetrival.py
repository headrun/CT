        ...
        try:
            ...
            if response.status == 200:
                ...
                if room_nodes:
                    ...
                    )).replace(u'\xa0', '').replace('Rs.', '').replace(',', '')
                    soldout_check = ''.join(
                ...
                rmtc = 'N/A'
                item_booking = self.get_yield(ct_id,  final_rate_plan, adult, child, dx, los, check_in, check_out, city_name, hot_name,
                                              h_id, reference_url, room_type, rate_plan, inclusions, cancellation_policy, splashed_price, actual_price, '', '', rmtc)
                if item_booking:
                    yield item_booking
            self.cursor.execute(
    ...
    def parse_nexts(self, response):
        ...
        if goods_service_tax:
            goods_service_tax = normalize_clean(goods_service_tax[0].encode(
                'ascii', errors='ignore').replace('Rs.', '').replace(',', ''))
        else:
            goods_service_tax = 0
        ...
        if city_tax:
            city_tax = normalize_clean(city_tax[0].encode('ascii', errors='ignore').replace('Rs.', '').replace(',', ''))
        else:
            city_tax = 0
        ...
        if cleaning_fee:
            cleaning_fee = normalize_clean(cleaning_fee[0].encode('ascii', errors='ignore').replace('Rs.', '').replace(',', '').replace('NOK', '').strip())
        else:
            cleaning_fee = 0
        ...
        if damage_deposit and 'refundable' not in refundable_check:
            damage_deposit = normalize_clean(damage_deposit[0].encode('ascii', errors='ignore').replace('Rs.', '').replace(',', ''))
        else:
            damage_deposit = 0
        ...
        if goods_service_tax == 0:
            ...
            if goods_service_tax:
                goods_service_tax =  normalize_clean(goods_service_tax[0].encode('ascii', errors='ignore').replace('Rs.', '').replace(',', ''))
            else:
                goods_service_tax = 0
