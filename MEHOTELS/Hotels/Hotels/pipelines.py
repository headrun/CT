...
class HotelsPipeline(object):
    def process_item(self, item, spider):
         ...
         if isinstance(item, ContentScore):
            conte_scor = '#<>#'.join([
                str(item['sk']), str(item.get('hotel_id', '')), str(item.get('hotel_name', '')), str(item.get('address', '')), str(item.get('city', '')), str(item.get('locality_latitude', '')), str(item.get('locality_longitude', '')), str(item.get('star_rating', '')), str(item.get('description', '')), str(item.get('amenities', '')), str(item.get('aux_info', '')), str(item.get('reference_url', '')), str(item.get('html_hotel_url', '')), str(item.get('main_listing_url', '')), str(item.get('navigation_url', ''))
            ])
            spider.out_put_file.write('%s\n' % conte_scor)
            spider.out_put_file.flush()
         return item
