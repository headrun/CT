...
class GOICrawltreterive(scrapy.Spider):
	...
	def parse(self,response):
		...
		for city_name,main_url,dx,los,pax,start_date,end_date,city_code,hotel_ids,hotel_name in rows:
			if main_url:
				yield Request(main_url,callback=self.parse_innerpage1,headers=headers,meta={'city_name':city_name.split("_")[0].strip(),'dx':dx,'los':los,'pax':pax,'start_date':start_date,'end_date':end_date,'city_code':city_code,'hotel_ids':hotel_ids,'hotel_name':hotel_name})
		...
		try:
			...
			if response.status==200:
				...
				if data and data!='':
					...
					if main_node is not None and parent_node is not None:
						if isinstance(parent_node,list):
							...
							if prime_rtc and prime_rpc and prime_rtc!='' and prime_rpc!='':
								...
								yield FormRequest(url,callback=self.parse_innerpage2,formdata=formdata1,meta={'city_name':city_name,'dx':dx,'los':los,'pax':pax,'check_in':check_in,'check_out':check_out,'city_code':city_code,'goid':goid,'hotel_name':hotel_name,'ref_url':main_url,'adults':adults,'prime_rtc':prime_rtc,'prime_rpc':prime_rpc,'formdata':json.dumps(formdata1)})
					...
					gob_items = GOBTRIPItem()
					gob_items.update({'city':normalize(city_name),'gbthotelname':normalize(hotel_name),'gbthotelid':goid,'check_in':check_in,'dx':dx,'los':los,'gbtpax':adults,'gbtroomtype':'N/A','gbtrate':'N/A','gbtb2cdiff':'N/A','gbtinclusions':'N/A','gbtapprate':'N/A','mobilediff':'N/A','gbtb2csplashedprice':'N/A','gbtappsplashedprice':'N/A','gbtb2ctaxes':'N/A','gbtapptaxes':'N/A','child':'0','gbtcouponcode':'N/A','gbtcoupondescription':'N/A','gbtcoupondiscount':'N/A','rmtc':'N/A','check_out':check_out,'gstincluded':'N/A','totalamtaftergst':'N/A','reference_url':main_url})
					if aux_info:
						gob_items.update({'aux_info':json.dumps(aux_info)})
			...
			if data and data!='':
				main_node = data.get('cdata',{})
						...
						coupon_description = 'N/A'
                                        ...
                                        coupon_description = 'N/A'
				goib_inclusions = u" ".join(main_node.get('Inclusions','')).replace(",","")\
				.replace(",    ","").decode('ascii','ignore').strip()
				...
				gob_items = GOBTRIPItem()
				gob_items.update({'city':normalize(city_name),'gbthotelname':normalize(hotel_name),'gbthotelid':goid,
				'check_in':check_in,'dx':dx,'los':los,'gbtpax':adults,'gbtroomtype':goib_roomtype,'gbtrate':total_after_gst,
				'gbtb2cdiff':total_after_gst,'gbtinclusions':normalize(goib_inclusions),'gbtapprate':'N/A','mobilediff':'N/A',
