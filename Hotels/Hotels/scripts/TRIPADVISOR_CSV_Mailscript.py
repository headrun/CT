...
class SendmailTA(object):
    ...
    def send_mails(self):
        ...
        sender, receivers  = 'kiranmayi@notemonk.com', ','.join(recievers_list)
        msg = MIMEMultipart('alternative')
        ccing = ['raja@headrun.com',
                        'hareesh.r@cleartrip.com', 'shyam.sundar@cleartrip.com',
                        'kora.rohan@cleartrip.com', 'rajul.kukreja@cleartrip.com', 'richa.s@cleartrip.com'

                 ]
	mbfile= self.humansize(self.size.st_size)
        if self.size.st_size <= 455:
	    istatus = "Incompleted"
            msg['Subject'] = 'TRIPADVISOE CSV Sheet Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> TRIPADVISOR CSV SHEET is not uploaded successfully:</h2>'
	    ...
	    cstatus='Completed'
            msg['Subject'] = 'TRIPADVISOR CSV Sheet  Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> TRIPADVISOR CSV SHEET uploaded successfully. </h2>'
	    mas+= '<table border="1">'
	    mas+='<tr><th>File Loction</th><th>CSV File Name </th><th>File Size in Bytes</th><th>Status</th><th>File Size In MB</th>'
        ...
        s.starttls()
        s.ehlo()
        s.login(sender, 'saibabaramanaiah427#')
