...
class Sendmail(object):
    ...
    def __init__(self):
        ...
        query = "select count(mmthotelid) from Makemytrip where created_on between concat(curdate(),' 01:35:00') and concat(curdate(), ' 01:50:00');"
	...
	ccing = ['raja@headrun.com', 'cleartrip@headrun.com',
        ...
        if self.ua_list[0][0]==0:
            msg['Subject'] = 'Makemytrip Terminal Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Makemytrip - Source table data fails to load :</h2>'
            ...
            msg['Subject'] = 'Makemytrip  Terminal Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Makemytrip -Source table  data is loading successfully.</h2>'
            msg['From'] = sender
            msg['To'] = receivers
            tem = MIMEText(''.join(mas), 'html')
            msg.attach(tem)
