...
class Sendmail(object):
    ...
    def send_mails(self):
        ...
        ccing = ['raja@headrun.com', 'cleartrip@headrun.com',
			...
			'kora.rohan@cleartrip.com', 'rajul.kukreja@cleartrip.com'
                 ]
	mbfile= self.humansize(self.size.st_size)
        if self.size.st_size <= 455:
	    istatus = "Incompleted"
            msg['Subject'] = 'GOIBIBO CSV Sheet Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> GOIBIBO CSV SHEET is not uploaded successfully:</h2>'
	    ...
	    cstatus='Completed'
            msg['Subject'] = 'GOIBIBO CSV Sheet  Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> GOIBIBO CSV SHEET uploaded successfully. </h2>'
	    mas+= '<table border="1">'
	    mas+='<tr><th>File Loction</th><th>CSV File Name </th><th>File Size in Bytes</th><th>Status</th><th>File Size In MB</th>'
