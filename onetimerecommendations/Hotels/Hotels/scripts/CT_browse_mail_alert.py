...
class Sendmail(object):
    ...
    def __init__(self):
        ...
        query = "select count(sk) from Cleartrip_crawl where created_at between concat(curdate(),' 00:30:00') and concat(curdate(), ' 01:45:00');"
    ...
    def send_mails(self):
        ...
        if self.ua_list[0][0]==0:
            msg['Subject'] = 'Cleartrip Browse Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Cleartrip -Crawl table data fails to load :</h2>'
            ...
            msg['Subject'] = 'Cleartrip Browse Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Cleartrip -Crawl table data  is loaded successfully. </h2>'
            msg['From'] = sender
            msg['To'] = receivers
            tem = MIMEText(''.join(mas), 'html')
            msg.attach(tem)
