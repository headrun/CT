...
class SendmailGONew(object):
	...
	try:
		...
		for csvfile in os.listdir(self.mydir):
			...
			self.all_files.append((csvfile, (os.stat(os.path.join(self.mydir, csvfile)).st_size)))
	...
	istatus = "Completed"
	msg['Subject'] = 'GOIBIBO/MMT COMMON CSV Sheet Availability on %s'%(str(datetime.datetime.now().date()))
	mas = '<h2> GOIBIBO/MMT COMMON CSV SHEET uploaded successfully. </h2>'
	mas+= '<table border="1">'
	...
	for all_file in self.all_files:
		mbfile= self.humansize(all_file[1])
        	if all_file[1] <= 455:
		    istatus = "Incompleted"
		mas+='<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>' % (self.mydir,all_file[0], all_file[1], istatus, mbfile)
	mas+='</table>\n'
	msg['From'] = sender
	msg['To'] = receivers
