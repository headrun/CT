...
class SendmailTG(object):
	...
	mbfile= self.humansize(self.size.st_size)
	    ...
	    istatus = "Incompleted"
            msg['Subject'] = 'TRIVAGO CSV Sheet Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> TRIVAGO CSV SHEET is not uploaded successfully:</h2>'
	    ...
	    cstatus='Completed'
            msg['Subject'] = 'TRIVAGO CSV Sheet  Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2> TRIVAGO CSV SHEET uploaded successfully. </h2>'
	    mas+= '<table border="1">'
	    mas+='<tr><th>File Loction</th><th>CSV File Name </th><th>File Size in Bytes</th><th>Status</th><th>File Size In MB</th>'
