...
class Sendmail(object):
    ...
    def __init__(self):
        ...
        query = "select count(gbthotelid) from Goibibotrip where created_on between concat(curdate(),' 00:30:00') and concat(curdate(), ' 01:50:00');"
    ...
    def send_mails(self):
        ...
        if self.ua_list[0][0]==0:
            msg['Subject'] = 'Goibibotrip Terminal Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Goibibotrip - Source table data fails to load :</h2>'
            ...
            msg['Subject'] = 'Goibibotrip  Terminal Availability on %s'%(str(datetime.datetime.now().date()))
            mas = '<h2>Goibibotrip -Source table  data is loading successfully. </h2>'
            msg['From'] = sender
            msg['To'] = receivers
            tem = MIMEText(''.join(mas), 'html')
            msg.attach(tem)
