...
class CrawlCTreteriveapi(scrapy.Spider):
    ...
    def parse_next_stage(self, response):
        ...
        if response.status == 200:
            ...
            if data:
                if not hotels_check:
                    ...
                    if child_single_node:
                        rmtc = ''.join(child_single_node.xpath(
                            './/room-type-code/text()').extract())
                        tax = ''.join(child_single_node.xpath(
                            './/pricing-element[category[contains(text(), "TAX")]]/amount/text()').extract())
                        try:
                            tax = str(int(round(float(tax))))
                        except:
                            tax = '0'
                        inclusions = ', '.join(child_single_node.xpath(
                            './/inclusion/text()').extract())
                        roomtype = ''.join(child_single_node.xpath(
                            './/room-description/text()').extract())
                        totalamt = child_single_node.xpath(
                            './/amount/text()').extract()
                        try:
                            totalamt = str(int(round(sum([float(i) for i in data.xpath(
                                '//room-rate')[0].xpath('.//amount/text()').extract()]))))
                        except:
                            totalamt = '0'
                        with_dis = child_single_node.xpath(
                            './/pricing-element[category[contains(text(), "DIS")]]/amount/text()').extract()
                        try:
                            with_dis = str(
                                int(round(sum([float(i) for i in with_dis])))).replace('-', '')
                        except:
                            with_dis = '0'

                        without_dis = child_single_node.xpath(
                            './/pricing-element[category[not(contains(text(), "DIS"))]]/amount/text()').extract()
                        try:
                            without_dis = str(
                                int(round(sum([float(i) for i in without_dis]))))
                        except:
                            without_dis = '0'
                        cancellation_policy = ', '.join(
                            child_single_node.xpath('.//cancel-policy/text()').extract())
            ...
            yield ct_items
        self.cursor.execute(
            "update %s_crawl set crawl_status=1 where sk = '%s'" % (self.name, sk_crawl))
