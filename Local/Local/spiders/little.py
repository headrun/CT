    ...
    def parse(self, response): 
        ...
        for k in list1:
            for i in x:
                ...
                city = i['cy']
                if city:
                    link = 'https://littleapp.in/api/search/v5?q={%22who%22:{%22p%22:%22+919999999999%22},%22s%22:{%22c%22:{%22l%22:{%22lat%22:'+str(lat)+',%22lon%22:'+str(lon)+'},%22gkw%22:%22'+k+'%22,%22pg%22:0}}}'
                    yield Request(link, callback = self.parse_next)
    def parse_next(self, response): 
        json_data = json.loads(response.body)['s']['results']
        m = json_data['m']
        for i in m:
            name = i.get('mn', '')
            sk = i.get('mc', '')
