...
class AirAsiaCheck:
	...
	conn = MySQLdb.connect(host = 'localhost', user = 'root', passwd = 'root', db = 'SEATFINDERDB', charset="utf8", use_unicode=True)
        ...
        path_to_move = current_path+ '/'+'AIRASIA_CSV/processed_complete'
        path_to_check = current_path +'/'+'AIRASIA_CSV/processed'
        ...
        with codecs.open(file_,'r',encoding='utf-8', errors='ignore') as csvfile:
            ...
            for row in plots:
                ...
                if 'booking_no' in row and "total_booking_amount" in row:
                    ...
                    if "Air Asia" in airline and gds_pnr=='':
				...
				cur.execute(seat_finder_query)
                		...
                		if len(rows)>=1:
					for row_db in rows:
						if "Unable To View PNR" in row_db[5]:
							print airline_pnr
							csv_list.append(row)
						elif 'Response Issue' in row_db[5]:
							print airline_pnr
							csv_list.append(row)
						elif 'Internal Server Error' in row_db[5]:
							print airline_pnr
							csv_list.append(row)
						elif 'Not within the range of conditions' in row_db[5]:
							print airline_pnr
							csv_list.append(row)
				else:
					csv_list.append(row)
	...
	os.system('scrapy crawl airasia_seatfinder_browse -ajsons="%s" --set HTTP_PROXY="%s" --set DOWNLOAD_DELAY=1'%(details_dict,ip))
