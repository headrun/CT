...
class SpiceJetCheck:
	...
	conn = MySQLdb.connect(host = 'localhost', user = 'root', passwd = 'root', db = 'SEATFINDERDB', charset="utf8", use_unicode=True)
        ...
        path_to_move = current_path+ '/'+'SPICEJET_CSV/processed_complete'
        path_to_check = current_path +'/'+'SPICEJET_CSV/processed'
        ...
        with codecs.open(file_,'r',encoding='utf-8', errors='ignore') as csvfile:
            ...
            for row in plots:
                ...
                if 'booking_no' in row and "total_booking_amount" in row:
                    ...
                    if airline=="SpiceJet" and gds_pnr=='':
				...
				cur.execute(seat_finder_query)
                		...
                		if len(rows)>=1:
					for row_db in rows:
                                                if "Itinerary page not present" in row_db[5]:
                                                        print airline_pnr
                                                        csv_list.append(row)
				else:
					csv_list.append(row)
	...
	os.system('scrapy crawl spicejet_seatfinder_browse -ajsons="%s" --set HTTP_PROXY="%s"'%(details_dict,ip))
