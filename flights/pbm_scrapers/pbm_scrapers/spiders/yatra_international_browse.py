...
class YatraInternationalBrowse(Spider):
    name = "yatra_international_browse"
    start_urls = ["https://www.yatra.com/"]

    def __init__(self, *args, **kwargs):
        super(YatraInternationalBrowse, self).__init__(*args, **kwargs)
	...
	'''
	from_, to_, date, re_date = 'DXB', 'BKK', '15/12/2017', '2017-12-25'
	...
	try:resultdata = body.get('resultData', [])
	...
	for res in resultdata:
		...
		val = [sk, fare_val, '<>'.join(airline_lst), de_dat, ar_dat, 'International', seg, 'oneway', '<>'.join(flt_ids)]
	...
	for i in sorted_x:
		...
		sk, fare, air_name, de_dat, ar_dat, seg_type, seg, trip_type, flts_ids = k
			...
			str(ar_dat), str(fin_rank), seg_type, str(seg), 'oneway', str(flts_ids))
