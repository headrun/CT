...
class CleartripIntBrowse(Spider):
    name = "cleartrip_international_browse"
    start_urls = ["https://www.cleartrip.com/flights/"]

    def __init__(self, *args, **kwargs):
        super(CleartripIntBrowse, self).__init__(*args, **kwargs)
	...
	'''
	from_, to_, date = 'DXB', 'BKK', '2017-12-15'
	#url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&country=IN&currency=INR&jsonVersion=1.0'%(from_, to_, date)
	url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&jsonVersion=1.0'%(from_, to_, date)
        headers = {
                'X-CT-API-KEY' : 'bce875d513bab5fa2cba6371f7b1ea58',
                'X-CT-SOURCETYPE' : 'B2C'
                }
	...
	for rank, ow in enumerate(oneway_mapping, 1):
	    ...
	    seg_price_order.append([sk, fare_, '<>'.join(air_name), de_dat, ar_dat, '0', '%s-%s'%(from_, to_), '<>'.join(flts_lst), sk])
	...
	for i in sorted_x:
		...
		vals = (str(sk), str(fare), str(air_name), str(de_dat), str(ar_dat), str(fin_rank), 'International', str(seg), 'oneway', str(flts_))
