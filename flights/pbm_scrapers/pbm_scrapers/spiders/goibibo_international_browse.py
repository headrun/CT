...
class GoibiboInternationalBrowse(Spider):
    name = "goibibo_international_browse"
    start_urls = ["https://www.goibibo.com/flights/"]

    def __init__(self, *args, **kwargs):
        super(GoibiboInternationalBrowse, self).__init__(*args, **kwargs)
	...
	headers = {
	    ...
	    'Referer': 'https://www.goibibo.com/flights/air-DXB-BKK-20171215--1-0-0-E-I/',
	...
	params = (
  			...
  			('flavour', 'v2'),
    			('mime', 'html'),
  			('script', 'y'),
    			('actionData', '[{"query":"air-DXB-BKK-20171215--1-0-0-E-0"}]'),
			('bookability', 'n'),
		)
	from_, to_, date, re_date = 'DXB', 'BKK', '15/12/2017', '2017-12-15'
	url = 'https://thor.goibibo.com/v2/thor/rest/flight/search/int'
	#url = 'https://thor.goibibo.com/v2/thor/rest/flight/search/int?userid=asd&application=fbs&flavour=v2&mime=html&script=y&actionData=[{%22query%22:%22air-DXB-BKK-20171215--1-0-0-E-0%22}]&bookability=n'
	date = '2017-12-15'
        yield FormRequest(url, callback=self.parse_next, formdata=params, meta={'d_date':date, 'from_':from_, 'to_':to_}, method='GET')
	#yield Request(url, callback=self.parse_next,  meta={'d_date':date, 'from_':from_, 'to_':to_}, method='GET')
	...
	for dit in row_data_lst:
		...
		val = [sk, price, '',  de_date_time.strip(), ar_date_time.strip(), 'International', seg, 'oneway', flight_ids, json.dumps(aux_info)]
	...
	for i in sorted_x:
		...
		sk, fare, air_name, de_dat, ar_dat, seg_type, seg, trip_type, flts_ids, aux_ = k
			...
			str(ar_dat), str(fin_rank), seg_type, str(seg), 'oneway', str(flts_ids), aux_)
