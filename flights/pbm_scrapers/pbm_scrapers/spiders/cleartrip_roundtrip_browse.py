...
class CleartripRoundTripBrowse(Spider):
	...
	'''
	#from_, to_, date, re_date = 'BOM', 'CCU', '2018-01-15', '2018-01-25'
	...
	#from_, to_, date, re_date = 'DXB', 'BKK', '2018-01-15', '2018-01-25'
	...
	url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&return-date=%s&adults=1&country=IN&currency=INR&jsonVersion=1.0'%(from_, to_, date, re_date)
        ...
        yield Request(url, callback=self.parse_next, headers=headers, meta={'d_date':date, 'from_':from_, 'to_':to_, 're_date':re_date})
	...
	if self.crawl_type == 'international':
	    seg_price_order = self.get_international_price_order(oneway_mapping, content, fares, date, seg, air_names, re_date)
	    ...
	    return_price_order = self.get_price_order_dict(return_mapping, content, fares, date, re_seg, air_names)
	    ow_sorted = self.get_sorted_values(seg_price_order)
	...
	for i in sorted_x:
            ...
            for k in lsttt:
                sk, fare, air_name, de_dat, ar_dat, rank, seg, flts_, sk, aux_info = k
                vals = (str(sk), str(fare), str(air_name), str(de_dat), \
			str(ar_dat), str(fin_rank), self.crawl_type.capitalize(), str(seg),
			self.trip_type.capitalize(), str(flts_), aux_info)
	...
	segments_lst, seg_price_order = [], []
	    ...
	    sk_date = str(datetime.datetime.now())
            sk = str(hashlib.md5('%s%s'%(str(cont_code_lst), sk_date)).hexdigest())
		...
		air_code = flight_no.split('-')[0].strip()
            ...
            seg_price_order.append([sk, fare_, '<>'.join(air_name), de_dat, ar_dat, '0', seg, '<>'.join(flts_lst), sk, json.dumps({})])
	return seg_price_order

    def get_international_price_order(self, oneway_mapping, content, fares, date, seg, air_names, re_date):
	segments_lst, seg_price_order = [], []
        temp_dict = {}
        for rank, ow in enumerate(oneway_mapping, 1):
            seg_details, status = {}, False
	    aux_info, return_details = {}, {}
            ...
            sk = str(hashlib.md5('%s%s'%(str(cont_code_lst), sk_date)).hexdigest())
            ar_seg_time, de_seg_time = '', ''
	    ow_map_lst, rt_map_lst = cont_code_lst
	    flts_lst, air_name, ar_seg_time, de_seg_time = self.get_flight_details(ow_map_lst, air_names, content)
	    re_flt_lst, re_air_name, re_ar_seg_time, re_de_seg_time = self.get_flight_details(rt_map_lst, air_names, content)
            ar_dat = '%s %s'%(date, ar_seg_time)
            de_dat = '%s %s'%(date, de_seg_time)
	    ...
	    aux_info.update({'return_details':return_details})
            seg_price_order.append([sk, fare_, '<>'.join(air_name), de_dat, ar_dat, '0', seg, '<>'.join(flts_lst), sk, json.dumps(aux_info)])
        return seg_price_order
	...
	for seg_idx, i in enumerate(map_lst):
	    ...
	    if airline: air_name.append(airline)
	    flts_lst.append(flight_no)
	return (flts_lst, air_name, ar_seg_time, de_seg_time)
