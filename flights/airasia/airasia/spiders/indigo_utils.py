...
class IndigoUtils(object):
	...
	def get_pcc_name(self):
		...
		if len(data)==1:
			pcc_name = 'indigo_%s' % data[0].keys()[0]
		elif len(data)==2:
	...
	def parse_flight_segments(self, seg_trips):
		...
		for seg_idx, seg in enumerate(seg_trips):
			...
			if len(i['legs']) > 1:
				all_flights = []
				for leg in i['legs']:
					all_flights.append(leg['flightDesignator']['flightNumber'].strip())
				flt_no = '<>6E'.join(all_flights)
	...
	def mealcode_inputs(self):
		...
		for index, i in enumerate(self.ow_input_flight):
			...
			if owmeal:
				owmeal = [(str(index) + '_' + str(count) + '_' + i) for count, i in enumerate(owmeal)]
		...
		for index, i in enumerate(self.rt_input_flight):
			...
			if rtmeal:
				rtmeal = [(str(index) + '_' + str(count) + '_' + i) for count, i in enumerate(rtmeal)]
	...
	def baggagecode_inputs(self):
		...
		for index, i in enumerate(self.ow_input_flight):
			...
			if ow_baggage:
				ow_baggage = [(str(index) + '_' + str(count) + '_' + i) for count, i in enumerate(ow_baggage)]
		...
		for index, i in enumerate(self.rt_input_flight):
			...
			if rt_baggage:
				rt_baggage = [(str(index) + '_' + str(count) + '_' + i) for count, i in enumerate(rt_baggage)]
	...
	def get_input_segments(self, segments):
		...
		if len(all_segments) == 1:
		    ...
		    vals = (segments.get('trip_ref', ''), 'Indigo', '', '', \
		    	segments.get('origin_code', ''), segments.get('destination_code', ''),\
		    	'', '', '', "Booking Failed", '', "Multi-city booking", \
		...
		if ow_len > 1 or rt_len > 1:
			vals = (segments.get('trip_ref', ''), 'Indigo', '', '', segments.get('origin_code', ''),\
					segments.get('destination_code', ''), '', '', '', "Booking Failed", '',\
		...
		for i in book_dict['onewaybaggagecode']:
			...
			try:
				...
				vals = (book_dict.get('tripid', ''), 'IndiGo', '', '', book_dict.get('origin', ''),\
						book_dict.get('destination', ''), book_dict.get('triptype', ''),\
						book_dict.get('ctprice', ''), '',\
						'Baggages error', '', 'Check oneway baggages', '', '{}',\
						'Check oneway baggages', '', book_dict.get('tripid', ''),''\
						)
				self.cur.execute(self.insert_query, vals)
				self.conn.commit()
				print e
		...
		for i in book_dict['returnbaggagecode']:
				...
				try:
					...
					vals = (book_dict.get('tripid', ''), 'IndiGo', '', '', book_dict.get('origin', ''),\
                        	...
                        	'Check return baggages', '', book_dict.get('tripid', ''),''\
                        	)
					self.cur.execute(self.insert_query, vals)
					self.conn.commit()
					print e
		...
		for i in book_dict['onewaymealcode']:
			...
			try:
				...
				vals = (book_dict.get('tripid', ''), 'IndiGo', '', '', book_dict.get('origin', ''),\
                		...
                		'Check oneway meals', '', book_dict.get('tripid', ''),''\
                		)
				self.cur.execute(self.insert_query, vals)
				self.conn.commit()
				print e
		...
		for i in book_dict['returnmealcode']:
			...
			try:
				...
				vals = (book_dict.get('tripid', ''), 'IndiGo', '', '', book_dict.get('origin', ''),\
						...
						'Check return meals', '', book_dict.get('tripid', ''),''\
						)
				self.cur.execute(self.insert_query, vals)
				self.conn.commit()
				print e
	...
	def check_tolerance(self, ctprice, indiprice):
		...
		if total_fare != 0:
		    ...
		    if tolerance_value >= float(self.booking_dict.get('tolerance_amount', '2000')):#2000:
	...
	def process_price_details(self, all_edit, price_details_major, book_dict):
		...
		from_key_value = '%s-%s' % (key_flights['items'][0]['segments']['items'][0]['departureStation'], key_flights['items'][0]['segments']['items'][0]['arrivalStation'])
		from_flight_key = '%s %s' % (key_flights['items'][0]['segments']['items'][0]['flightDesignator']['carrierCode'], key_flights['items'][0]['segments']['items'][0]['flightDesignator']['flightNumber'])
		if self.ow_flights_connection:
			...
			from_flight_key = '%s %s' % (key_flights['items'][0]['segments']['items'][0]['flightDesignator']['carrierCode'], '<>6E'.join(all_from_flights))
			from_key_value = from_key_value + '-' + '%s-%s' % (key_flights['items'][0]['segments']['items'][1]['departureStation'], key_flights['items'][0]['segments']['items'][1]['arrivalStation'])
		if self.trip_type == 'RoundTrip':
			to_key_value = '%s-%s' % (key_flights['items'][1]['segments']['items'][0]['departureStation'], key_flights['items'][1]['segments']['items'][0]['arrivalStation'])
			to_flight_key = '%s %s' % (key_flights['items'][1]['segments']['items'][0]['flightDesignator']['carrierCode'], key_flights['items'][1]['segments']['items'][0]['flightDesignator']['flightNumber'])
			if self.rt_flights_connection:
				all_to_flights = []
				for i in key_flights['items'][1]['segments']['items']:
					all_to_flights.append(i['flightDesignator']['flightNumber'])
					to_flight_key = '%s %s' % (key_flights['items'][1]['segments']['items'][0]['flightDesignator']['carrierCode'], '<>6E'.join(all_to_flights))
					to_key_value = to_key_value + '-' + '%s-%s' % (key_flights['items'][1]['segments']['items'][1]['departureStation'], key_flights['items'][1]['segments']['items'][1]['arrivalStation'])
		all_keys = []
		if len(key_flights) > 2:
			return price_details_major
		#totalFare = all_edit['indiGoPriceBreakdown']['indigoJourneyPrice']['totalPrice']
		baggage_keys, meal_keys, ow_baggage_price, rt_baggage_price, ow_meals_price, rt_meals_price = self.get_baggage_meals(all_edit, book_dict)
		print baggage_keys, meal_keys
		for index, edit in  enumerate(edits):
			edit_pi = edit['journeyPriceItinerary']
			price_details = {}
			try:
				adult_child_base = float(float(edit_pi['baseFare'])/(int(self.adult_count)+ int(self.child_count)))
			except:
				print 'Error'
				vals = (book_dict.get('tripid', ''), 'IndiGo', '', '', book_dict.get('origin', ''),\
						book_dict.get('destination', ''), book_dict.get('triptype', ''),\
						book_dict.get('ctprice', ''),\
						'', 'Booking failed', '', 'Error with adult child calc', '', json.dumps(price_details_major),\
						'Error with adult child calc', '', book_dict.get('tripid', ''), json.dumps(price_details_major)\
						)
			...
			if self.infant_count != '0':
			    try:
				    price_details.update({'Infant' : edit_pi['ssrAmountList'][0]['value']})
			    except:
				    pass
			if index == 0:
				if ow_baggage_price:
			...
			if index == 0:
				...
				price_details.update({'pcc' : self.booking_dict['all_segments'][0].keys()[0]})
				price_details_major.update({from_flight_key : price_details})
			elif index == 1:
				price_details.update({'seg' : to_key_value})
				price_details.update({'pcc' : self.booking_dict['all_segments'][1].keys()[0]})
