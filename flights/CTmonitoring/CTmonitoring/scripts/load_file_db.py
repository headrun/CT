...
class LoadFiles(object):
	...
	self.insert_query = 'insert into Availability (sk, flight_id, date, type, dx, no_of_passengers, is_available, airline, departure_time, arrival_time, from_location, to_location, providers, aux_info, reference_url, created_at, modified_at) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, now(), now()) on duplicate key update sk=%s, providers=%s, is_available=%s'
        self.main()
	...
	for rec in data:
	    ...
	    if rec:
	        ...
	        vals = (
				...
				dic['providers'], dic['aux_info'], dic['reference_url'], dic['sk'], dic['providers'],
				dic['is_available'],
			)
