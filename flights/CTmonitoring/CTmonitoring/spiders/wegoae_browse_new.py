...
class WegoAeNewBrowse(Spider):
    name = "wegoaenew_browse"
    start_urls = ["https://www.wego.ae/en"]

    def __init__(self, *args, **kwargs):
        super(WegoAeNewBrowse, self).__init__(*args, **kwargs)
	...
	requests = terminal_requests(self.cr_tabe, self.source_name, self.crawl_type, self.trip_type, self.limit)
	    ...
	    to_ = to_.strip()
            date = str(date.date())
	    refer_url = 'https://www.wego.ae/en/flights/searches/%s-c%s/%s/economy/1a:0c:0i'%(from_, to_, date)
	    headers = {
