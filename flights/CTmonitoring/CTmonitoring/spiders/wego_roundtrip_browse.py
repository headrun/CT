...
class WegoRTBrowse(Spider):
    name = "wegocort_browse"
    start_urls = ["https://www.wego.co.in/"]

    def __init__(self, *args, **kwargs):
        super(WegoRTBrowse, self).__init__(*args, **kwargs)
	self.source_name = 'wegort'
	self.crawl_table_name = 'wego'
	...
	requests = terminal_requests(self.cr_tabe, self.crawl_table_name, self.crawl_type, self.trip_type, self.limit)
	    ...
	    refer_url = 'https://www.wego.co.in/flights/searches/c%s-c%s/%s:%s/economy/1a:0c:0i'%(from_, to_, date, re_date)
	    headers = {
	...
	for lst in trip_list:
	    ...
	    url = "https://srv.wego.com/v2/metasearch/flights/trips/%s?currencyCode=INR"%ids
