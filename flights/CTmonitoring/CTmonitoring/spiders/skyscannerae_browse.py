...
class SkyAeBrowse(Spider):
    ...
    start_urls = ["https://www.skyscanner.net/"]
	...
	for input_ in requests:
	    ...
	    headers = {
			'x-skyscanner-devicedetection-istablet': 'false',
                    	...
                    	'referer': 'https://www.skyscanner.net/transport/flights/%s/%s/%s'%(from_.lower(), to_.lower(), refer_date),
	    ...
	    data = {
			...
			"trip_type":"one-way",
			"legs":[
					{"origin":from_,"destination":to_,"date":date}
				],
			"adults":1,
			"child_ages":[],
			"options":{
