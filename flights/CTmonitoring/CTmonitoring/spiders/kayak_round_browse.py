...
class KayakRtBrowse(Spider):
    ...
    def parse(self, response):
        ...
        for input_ in requests:
            ...
            yield FormRequest(url, callback=self.parse_next, cookies=cookies, meta={'date':date, 'got_sk': got_sk, 'from_':from_, 'to_':to_, 'dx':dx, 're_date':re_date})
    ...
    def parse_next(self, response):
        ...
        script_meta = ''.join(sel.xpath('//style[@data-type="script-metadata2"]//text()').extract()).replace('/n','')
        style_meta = ''.join(sel.xpath('//style[@data-type="style-metadata2"]//text()').extract()).replace('/n','')
        for nod in flight_nodes:
            ...
            d_to_city = ''.join(nod.xpath('.//ol[@class="flights"]//li[1]//div[@class="col-field time return"]\
				/div[@class="bottom"]/text()').extract())
            d_stop_count = len(nod.xpath('.//ol[@class="flights"]//li[1]//div[@class="col-field stops"]//\
				span[@class="dot-inner"]/@class').extract())
            ...
            r_to_city = ''.join(nod.xpath('.//ol[@class="flights"]//li[2]//div[@class="col-field time return"]\
                                /div[@class="bottom"]/text()').extract())
            r_stop_count = len(nod.xpath('.//ol[@class="flights"]//li[2]//div[@class="col-field stops"]//\
                                span[@class="dot-inner"]/@class').extract())
            ...
            yield FormRequest(post_url, formdata=form_data, callback=self.parse_provider, \
	    	    meta={'data':details, 'date':date, 'ref_url':response.url, 'dx':response.meta['dx'], 'o_stop_count':d_stop_count, 'r_stop_count':r_stop_count})
        if flight_nodes:
    ...
    def parse_provider(self, response):
        ...
        o_stop_count = response.meta['o_stop_count']
        r_stop_count = response.meta['r_stop_count']
        provider_dict, avail, aux_info = {}, {}, {}
        is_avail, rank = 0, 0
        airline = data.get('airline', '')
        from_ = data.get('from_', '')
        to_ = data.get('to_', '')
        oneway_journey = from_ + ' - ' +  to_
        return_journey = to_ + ' - ' + from_
        re_date = data.get('re_date', '')
        return_airline = data.get('return_airline', '')
        if o_stop_count: aux_info.update({'oneway_stop_count':o_stop_count})
        if r_stop_count: aux_info.update({'return_stop_count':r_stop_count})
        d_flight_id = ''.join(sel.xpath('//span[@class="origin-destination"][contains(text(),"%s")]/../../../div//div[@class="planeDetails details-subheading"]/text()' % oneway_journey).extract())
	r_flight_id = ''.join(sel.xpath('//span[@class="origin-destination"][contains(text(),"%s")]/../../../div//div[@class="planeDetails details-subheading"]/text()' % return_journey).extract())
        if d_flight_id: d_flight_id = re.search('\d+', d_flight_id).group()
        if r_flight_id: r_flight_id= re.search('\d+', r_flight_id).group()
        nodes = sel.xpath('//div[@class="content-card"]//table//tbody//tr')
        for idx, node in enumerate(nodes, 1):
            provider = ''.join(node.xpath('./td[@class="logo"]/img/@title').extract())
            price = normalize(''.join(node.xpath('./td[@class="price total"]/text()').extract()).replace(u'\u20b9\xa0', ''))
