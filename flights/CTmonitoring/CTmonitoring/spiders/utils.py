...
def insert_crawl_tables_data(cursor, source, val):
    ...
    CRAWL_TABLE_QUERY += ' ON DUPLICATE KEY UPDATE sk=%s, from_location=%s, to_location=%s, start_date=%s, crawl_type=%s, crawl_status=%s, modified_at=NOW()'
    if val:
	vals = (val['sk'], val['dx'], val['crawl_type'], val['from'], val['to'], val['date'],
		...
		val['date'], val['crawl_type'], val['crawl_status'],
	)
