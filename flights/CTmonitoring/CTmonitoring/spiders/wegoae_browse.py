...
class WegoAeBrowse(Spider):
    name = "wegoae_browse"
    start_urls = ["https://www.wego.ae/en"]

    def __init__(self, *args, **kwargs):
        super(WegoAeBrowse, self).__init__(*args, **kwargs)
	self.source_name = 'wegoae'
	...
	self.out_put = get_output_file(self.source_name)
	...
	self.out_put.close()
	self.cr_tabe.close()
	move_crawlout_processing(self.source_name)

    def parse(self, response):
        sel = Selector(response)
	requests = terminal_requests(self.cr_tabe, self.source_name, self.crawl_type)
        if requests:
		...
		date = str(date.date())
                ...
                url = 'https://www.wego.ae/en/flights/search'
	...
	if nodes:
	    got_page(self.cr_tabe, self.source_name, got_sk, 1, self.crawl_type)
