...
class KayakBrowse(Spider):
    ...
    def parse(self, response):
        ...
        for input_ in requests:
            ...
            yield FormRequest(url, callback=self.parse_next, cookies=cookies, meta={'date':date, 'got_sk': got_sk, 'from_':from_, 'to_':to_, 'dx':dx, })
    ...
    def parse_next(self, response):
        ...
        script_meta = ''.join(sel.xpath('//style[@data-type="script-metadata2"]//text()').extract()).replace('\n','')
        style_meta = ''.join(sel.xpath('//style[@data-type="style-metadata2"]//text()').extract()).replace('\n','')
        for nod in flight_nodes:
            ...
            count = len(nod.xpath('.//ol[@class="flights"]//span[@class="axis"]//span[@class="dot-inner"]/@class').extract())
            ...
            yield FormRequest(post_url, formdata=form_data, callback=self.parse_provider, meta={'data':details, 'date':date, 'ref_url':response.url, 'dx':response.meta['dx'], 'count':count})
    ...
    def parse_provider(self, response):
        ...
        is_avail, rank = 0, 0
        airline = normalize(data.get('airline', ''))
        from_ = normalize(data.get('from_', ''))
        to_ = normalize(data.get('to_', ''))
        if count:
            aux_info.update({'stop_count':count})
        flight_id = ''.join(sel.xpath('//div[@class="planeDetails details-subheading"]//text()').extract()).replace('\n', '')
