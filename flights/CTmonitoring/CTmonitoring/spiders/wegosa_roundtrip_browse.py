...
class WegoSaRTBrowse(Spider):
    name = "wegosart_browse"
    start_urls = ["https://sa.wego.com/en"]

    def __init__(self, *args, **kwargs):
        super(WegoSaRTBrowse, self).__init__(*args, **kwargs)
	self.source_name = 'wegosart'
	self.crawl_table_name = 'wegosa'
	...
	requests = terminal_requests(self.cr_tabe, self.crawl_table_name, self.crawl_type, self.trip_type, self.limit)
	    ...
	    refer_url = 'https://sa.wego.com/en/flights/searches/c%s-c%s/%s:%s/economy/1a:0c:0i'%(from_, to_, date, re_date)
	    headers = {
