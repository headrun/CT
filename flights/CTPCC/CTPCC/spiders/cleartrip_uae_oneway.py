...
class CleartripUaeBrowse(Spider):
    name = "cleartrip_uae_oneway"
    start_urls = ["https://www.cleartrip.com/flights/"]
    def __init__(self, *args, **kwargs):
        super(CleartripUaeBrowse, self).__init__(*args, **kwargs)
	self.source_name = 'cleartripuae'
	...
	for input_ in requests:
            ...
            to_ = to_.strip()
	    date = str(date.date())
	    url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&country=AE&currency=SAR&jsonVersion=1.0'%(from_, to_, date)
	    headers = {
	...
	for rank, ow in enumerate(oneway_mapping, 1):
	    ...
	    from_loc, to_loc = ['']*2
	    for _idx, j in enumerate(cont_code_lst):
		for seg_idx, i in enumerate(j):
		    i = content.get(i, {})
		    flt_dict = {}
		    ar_date = i.get('ad', '')
		    ar_time = i.get('a', '')
