...
class CleartripIndBrowse(Spider):
    name = "cleartrip_ind_oneway"
    start_urls = ["https://www.cleartrip.com/flights/"]
    def __init__(self, *args, **kwargs):
        super(CleartripIndBrowse, self).__init__(*args, **kwargs)
	self.source_name = 'cleartripind'
	...
	for input_ in requests:
            ...
            to_ = to_.strip()
	    date = str(date.date())
	    url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&country=IN&currency=SAR&jsonVersion=1.0'%(from_, to_, date)
	    headers = {
		'X-CT-API-KEY' : 'bce875d513bab5fa2cba6371f7b1ea58',
		'X-CT-SOURCETYPE' : 'B2C'
		}
	    yield Request(url, callback=self.parse_next, headers=headers, meta={'d_date':date, 'from_':from_, 'to_':to_, 'dx':dx})
	'''
	from_, to_, date, re_date = 'DMM', 'COK', '2017-11-22', '2017-12-25'
	url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&country=IN&currency=SAR&jsonVersion=1.0'%(from_, to_, date)
        headers = {
                'X-CT-API-KEY' : 'bce875d513bab5fa2cba6371f7b1ea58',
                'X-CT-SOURCETYPE' : 'B2C'
	...
	for rank, ow in enumerate(oneway_mapping, 1):
	    ...
	    from_loc, to_loc = ['']*2
	    for _idx, i_lst in enumerate(cont_code_lst):
		for seg_idx, i in enumerate(i_lst):
		    i = content.get(i, {})
		    flt_dict = {}
		    ar_date = i.get('ad', '')
		    ar_time = i.get('a', '')
		    ...
		    ar_dest = i.get('to', '')
		    flt_key = i.get('fk', '')
		    flight_no = ''.join(re.findall('_(\w{2}-\d+)_', flt_key)).strip()
		    airline = ''.join(re.findall('(.*)_\w{2}-\d+_', flt_key)).replace('_', ' ').strip()
	    ...
	    seg_price_order.append([sk, fare_, '<>'.join(air_name), de_dat, ar_dat, '0', '%s-%s'%(from_, to_), '<>'.join(flts_lst), sk, '1', str(no_of_stops)])
	...
	for i in sorted_x:
	    ...
	    for k in lsttt:
		sk, fare, air_name, de_dat, ar_dat, rank, seg, flts_, sk, no_pax, no_stops = k
		vals = (str(sk), str(fare), str(air_name), str(de_dat), str(ar_dat), str(fin_rank), 'Domestic', str(seg), 'OneWay', str(flts_), no_pax, no_stops, str(dx))
