...
class CleartripKsaBrowse(Spider):
	...
	for input_ in requests:
            ...
            to_ = to_.strip()
	    date = str(date.date())
	    url = 'https://api.cleartrip.com/air/1.0/search?from=%s&to=%s&depart-date=%s&adults=1&country=SA&currency=SAR&jsonVersion=1.0'%(from_, to_, date)
	    headers = {
